...
class TiRefuseLast20cViewController: UIViewController {
    ...
    @objc func playvideo() {
        guard let filePath = Bundle.main.path(forResource: "TiRefuseEnd20c", ofType: "mp4") else {
            print("動画が見つからない")
            return
        }
    ...
    @objc func goToNextScreen() {
        ...
        if let HomeContloller = storyboard.instantiateViewController(withIdentifier: "Home") as? ViewController{
            HomeContloller.modalTransitionStyle = .crossDissolve
            HomeContloller.modalPresentationStyle = .fullScreen
            present(HomeContloller, animated: false)
        }
//        // 次の画面に遷移
//        let nextViewController = TiGiveLastViewController()
